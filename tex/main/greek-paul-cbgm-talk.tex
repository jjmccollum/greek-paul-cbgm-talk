...
%		\begin{figure}
%			\centering
%			\includegraphics[width=0.3\textwidth]{../img/max-and-moritz-in-biblical-greek.png}
%			\quad
%			\includegraphics[width=0.35\textwidth]{../img/the-tale-of-peter-rabbit-in-koine-greek.png}
%		\end{figure}
%	\end{frame}
	...
	\begin{frame}
		\begin{itemize}
			...
			\item $a = b$: path of length $0$
			\item $a > b$: path of length $> 0$ from $a$ to $b$
			\item NOREL: no path from $a$ to $b$, but both have a \emph{common ancestor}
			\item UNCL: same as NOREL, but no common ancestor (reserved for when we don't know where $a$ or $b$ fits in the stemma)
		\end{itemize}
		\begin{center}
			\includegraphics[scale=0.5]{../img/B05K2V5U26-30-local-stemma.pdf}
			\includegraphics[scale=0.5]{../img/B05K2V5U26-30-local-stemma-path-2.pdf}
			\includegraphics[scale=0.5]{../img/B05K2V5U26-30-local-stemma-path-3.pdf}
		\end{center}
	\end{frame}
	\section*{Witnesses}
	\begin{frame}
